...
public class PinYinUtil {
		...
		for (int i = 0; i < nameChar.length; i++) {
			if (nameChar[i] > 128) {
				try {
					...
					log.error("to first Spell error!",e);
					throw e;
				}
			} else {
				pinyinName.append(nameChar[i]);
	 ...
	 * @throws BadHanyuPinyinOutputFormatCombination 
	 */
	public static String converterToSpell(String chines,boolean falg) throws BadHanyuPinyinOutputFormatCombination {
		StringBuffer pinyinName = new StringBuffer();
		char[] nameChar = chines.toCharArray();
		HanyuPinyinOutputFormat defaultFormat = new HanyuPinyinOutputFormat();
		defaultFormat.setCaseType(HanyuPinyinCaseType.LOWERCASE);
...
//		System.out.println(converterToFirstSpell("只取一个发音，如果是多音字?仅取第一个发音？").toUpperCase());
	}
}